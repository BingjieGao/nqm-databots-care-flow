module.exports = (function() {
  ...
  const areaServiceDatabot = function(input, output, context, deststream) {
    ...
    tdxApi.getDatasetDataAsync(areaServiceDataset, {areaServiceId}, null, null)
      .then((result) => {
        if (!result || result.data.length !== 1) {
          ...
          // requesting a set of available services as a list from networkId
      ...
      .then((response) => {
        if (!response || response.data.length === 0) {
          ...
          return mapData(response.data, ratioData, serviceArray, areaServiceId, deststream);
        }
      })
      .catch((err) => {
